...
**Export Evidence Package** — generates a timestamped, tamper-evident document containing the canonical transaction record, this journal entry, the mapping rule version, the full posting history, and any reconciliation or flag records. → See [evidence packages](../04-reconciliation/evidence-packages.md)
...
If you see a `failed` entry with a validation-related error, check whether the mapping rule's credit account percentages still sum to 100. → See [retrying failed entries](retrying-failed-entries.md)
