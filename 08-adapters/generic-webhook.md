...
If your source system requires a custom authentication handshake, has a non-JSON format, or needs conditional type mapping logic, consider [building a custom adapter](building-an-adapter.md) instead.
...
XML, form-encoded, and other non-JSON formats are not supported by the generic adapter. Build a [custom adapter](building-an-adapter.md) for those.
