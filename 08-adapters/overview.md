...
→ See [Settings → Adapters](../07-settings/adapters.md)
...
→ See [adapter specification](adapter-spec.md) for the technical contract
...
- [Adapter specification](adapter-spec.md) — the full interface contract that all adapters must implement
- [Building an adapter](building-an-adapter.md) — a practical walkthrough for developers
- [Generic webhook](generic-webhook.md) — configuring the `webhook-adapter` for your source system
- [Generic CSV import](generic-csv.md) — importing transaction files with the `csv-import` adapter
- [Generic poll](generic-poll.md) — polling an API with the `poll-adapter`
- [Zoho Books](zoho-books.md) — configuring the Zoho Books outbound adapter
- [Journal CSV export](journal-csv-export.md) — exporting journal entries as CSV for manual import
